...
const sqlQueries = [
  // Task 2.5.1: Tasks table policies
  ...
  // Task 2.5.4: Missing profiles fields
  ...
  // Task 2.5.5: attendance_above_90 BOOLEAN column
  `ALTER TABLE sessions
    ADD COLUMN IF NOT EXISTS attendance_above_90 BOOLEAN;`,

  // Task 2.5.6: Test 1 columns
  ...
  // Task 2.5.6: Backfill existing Test 1 data
  ...
  // Task 2.5.7: mentor_assignments fields
  ...
  // Task 2.5.8: sync trigger function
  `CREATE OR REPLACE FUNCTION sync_onboarding_to_profile()
   ...
   BEGIN
     UPDATE profiles SET
       father_occupation         = COALESCE(d->'family'->'father'->>'occupation', d->>'father_occupation', d->>'fatherOccupation'),
       father_education          = COALESCE(d->'family'->'father'->>'education', d->>'father_education', d->>'fatherEducation'),
       father_address            = COALESCE(d->'family'->'father'->>'address', d->>'father_address', d->>'fatherAddress'),
       mother_occupation         = COALESCE(d->'family'->'mother'->>'occupation', d->>'mother_occupation', d->>'motherOccupation'),
       mother_education          = COALESCE(d->'family'->'mother'->>'education', d->>'mother_education', d->>'motherEducation'),
       local_guardian_name       = COALESCE(d->'family'->'local_guardian'->>'name', d->>'local_guardian_name', d->>'guardianName'),
       local_guardian_phone      = COALESCE(d->'family'->'local_guardian'->>'contact', d->'family'->'local_guardian'->>'phone', d->>'local_guardian_phone', d->>'guardianPhone'),
       local_guardian_address    = COALESCE(d->'family'->'local_guardian'->>'address', d->>'local_guardian_address', d->>'guardianAddress'),
       local_guardian_occupation = COALESCE(d->'family'->'local_guardian'->>'occupation', d->>'local_guardian_occupation', d->>'guardianOccupation'),
       ...
       father_name               = COALESCE(d->'family'->'father'->>'name', d->>'father_name', d->>'fatherName'),
       father_contact_no         = COALESCE(d->'family'->'father'->>'contact', d->'family'->'father'->>'contact_no', d->>'father_contact_no', d->>'fatherContact'),
       mother_name               = COALESCE(d->'family'->'mother'->>'name', d->>'mother_name', d->>'motherName'),
       mother_contact_no         = COALESCE(d->'family'->'mother'->>'contact', d->'family'->'mother'->>'contact_no', d->>'mother_contact_no', d->>'motherContact')
...
async function tryConnection(region) {
  ...
  try {
    ...
    // Silent fail to allow scanning
    return null;
  }
}

async function main() {
  console.log('Finding a working database pooler host...');
  let client = null;
  ...
  if (!client) {
    console.error('❌ Error: Could not connect to any database pooler regions.');
    process.exit(1);
  }
  ...
  for (let i = 0; i < sqlQueries.length; i++) {
    ...
    try {
      ...
      console.error(`❌ Query ${i + 1}/${sqlQueries.length} failed: ${err.message}`);
    }
  }
