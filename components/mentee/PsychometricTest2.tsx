...
// Mock 30 items for brevity as full text isn't provided, just 30 yes/no/change items
const TEST_2_ITEMS = Array.from({ length: 30 }, (_, i) => `Item ${i + 1}: Self-assessment behavior statement...`)

export default function PsychometricTest2({ studentId, initialData, userRole }: { studentId: string, initialData: any, userRole: string }) {
  const isMentor = userRole === 'mentor'
  // Student can fill. If it exists, they can potentially edit it unless locked, but requirements say "student fills"
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    TEST_2_ITEMS.forEach((_, i) => {
      const val = formData.get(`item_${i}`)
      if (val) responses[`item_${i}`] = val as string
    })

    try {
      const res = await fetch(`/api/students/${studentId}/psychometric`, {
        method: 'POST', // Assuming POST handles upsert or insertion based on API logic
      ...
      setSuccess('Test 2 saved successfully')
    } catch (err: any) {
