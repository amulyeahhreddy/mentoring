...
export async function POST(
  ...
  try {
    ...
    if (profile?.role !== 'mentor') {
      return NextResponse.json({ error: 'Forbidden. Only mentors can sign-off sessions.' }, { status: 403 })
    }

    // Verify the session belongs to the mentor
    const { data: session, error: sessionError } = await adminClient
      .from('sessions')
      .select('mentor_id, session_status')
      .eq('id', sessionId)
      .single()

    if (sessionError || !session) return NextResponse.json({ error: 'Session not found' }, { status: 404 })
    if (session.mentor_id !== user.id) return NextResponse.json({ error: 'Forbidden' }, { status: 403 })
